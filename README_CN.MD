...
## 🇨🇳 本 Fork 的改动：内置高质量中文翻译词库（安装即用）

本 Fork（分支 `improved-zh-translation`）在原版基础上内置了一份约 **43.8 万条**的合并优化中文翻译词库 `danbooru.zh_ALL.csv`，并**默认启用**——按原版方式安装本 Fork 后无需任何设置，提示词下方的中文翻译（本地 CSV 翻译）直接生效。
